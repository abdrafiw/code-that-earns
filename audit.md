...
[] Submission creation checks a custom auth claim that the application never sets
...
[] A user can change their own role and promote themselves to a company
...
[] Bounty ownership can be forged during creation
...
[] Submission ownership and bounty validity can be forged during creation
...
[] Company submission updates are effectively unrestricted
...
[] Transaction creation is insecure for a financial record
...
[] All authenticated users can read every user profile
...
[] The payment service exposes an API credential to browser code and uses the wrong environment API
...
[] Creating a bounty invalidates the wrong React Query cache
...
[] Developer bounty filters have state and controls but do not filter anything
...
[] The public bounty query silently shows only the first 20 records

Evidence: `getAllBounties()` supports a cursor and `hasMore`, but `useGetBounty()` calls it once and discards `lastDoc` and `hasMore`.
...
[] Bounty-detail failures are converted into a false "not found" state
...
[] Auth profile loading can preserve a stale user or incorrectly appear logged out
...
[] Signup can leave an orphaned Firebase Auth account
...
[] Login silently creates a developer profile when a profile is missing
...
[] Logout clears local state before Firebase confirms sign-out
...
[] Submit-solution success has incomplete UX and failures are not shown
...
[] Duplicate submissions are not prevented
...
[] Company search excludes all legacy bounties without `searchTerms`
...
[] Company KPI values represent filtered rows but are labeled as global totals
...
[] Firestore backend search issues requests for intermediate keystrokes
...
[] Firestore profile and content data is trusted through unchecked type assertions
...
[] Header uses an effect and state for a value derived entirely from context
...
[] AppContext contains dead global state
...
[] The context provider value is recreated on every provider render
...
[] Route protection is scattered and inconsistent
...
[] There is no router error boundary or catch-all route
...
[] Query hooks expose custom `loading`/`error` shapes inconsistently
...
[] Query-key naming is inconsistent and not centralized
...
[] The company bounty hook is outside its feature and duplicates bounty mapping
...
[] Bounty hook names are singular even though they fetch collections

Evidence: `getBounty`/`useGetBounty` call `getAllBounties()` and return an array.

Solution: rename them to `getBounties`/`useGetBounties`; reserve singular names for ID-based queries.

[] Form state and validation are duplicated and weakly typed
...
[] The signup role is initialized as an invalid empty string and cast into `UserRole`
...
[] Several semantic/accessibility states have incorrect layout and missing actions
...
[] Firebase Analytics is initialized unconditionally
...
[] Firestore query indexes are manually enumerated for every filter combination
...
[] ESLint scans generated coverage output and important rules are disabled
